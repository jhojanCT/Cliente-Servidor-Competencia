...
class ProduccionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            if ($materiaPrima->stock < $request->cantidad_utilizada) {
                throw new \Exception("No hay suficiente materia prima filtrada disponible");
            }

            $costoProduccion = ($request->cantidad_utilizada * $materiaPrima->costo_promedio) + ($request->costo_adicional ?? 0);
    ...
    {
        $produccion->load(['materiaPrimaFiltrada', 'producto']);
    ...
    public function update(Request $request, Produccion $produccion)
    {
        ...
        DB::transaction(function () use ($request, $produccion) {
            ...
            $materiaPrimaAnterior = MateriaPrimaFiltrada::find($produccion->materia_prima_filtrada_id);
            $materiaPrimaAnterior->increment('stock', $produccion->cantidad_utilizada);
    ...
    public function destroy(Produccion $produccion)
    {
        DB::transaction(function () use ($produccion) {
            // Revertir stocks
            $materiaPrima = MateriaPrimaFiltrada::find($produccion->materia_prima_filtrada_id);
            $materiaPrima->increment('stock', $produccion->cantidad_utilizada);
